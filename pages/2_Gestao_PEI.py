...
# --- 0. INICIALIZAÇÃO DO BANCO DE DADOS (NOVO: Para integrar com o Adaptador) ---
...
# ABAS (Adicionada a aba "Salvar" no final)
abas = ["Início", "Estudante", "Mapeamento", "Plano de Ação", "Assistente de IA", "Documento", "💾 Salvar"]
...
with tab7:
    ...
    st.info("Ao salvar, os dados deste estudante ficarão disponíveis para o módulo de **Adaptação de Avaliações**.")

    if st.button("💾 Salvar Aluno no Sistema", type="primary"):
        if st.session_state.dados['nome']:
            # Cria uma cópia dos dados para não vincular com o formulário atual
            novo_aluno = st.session_state.dados.copy()
            
            # Adiciona ao banco
            st.session_state.banco_estudantes.append(novo_aluno)
            st.success(f"Sucesso! O estudante **{novo_aluno['nome']}** foi salvo e já pode ter suas provas adaptadas.")
        else:
            st.warning("É necessário preencher pelo menos o NOME do estudante antes de salvar.")
