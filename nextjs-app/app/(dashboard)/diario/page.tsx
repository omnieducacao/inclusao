...
export default async function DiarioPage({ searchParams }: Props) {
  ...
  if (workspaceId && studentId && !student) {
    ...
    if (studentFromList) {
      console.warn("⚠️ Diario: Estudante não encontrado por getStudent, mas encontrado na lista. Usando dados da lista.", {
        workspaceId,
        studentId,
      ...
      console.error("❌ Diario: Estudante não encontrado após getStudent e não está na lista", {
        workspaceId,
        studentId,
