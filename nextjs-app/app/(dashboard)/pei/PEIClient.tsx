...
function LaudoPdfSection({
  ...
  async function extrair() {
    ...
    try {
      ...
      const res = await fetch("/api/pei/extrair-laudo", { method: "POST", body: formData });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Erro ao extrair dados do laudo.");
      setExtraido({
        diagnostico: data.diagnostico || "",
        medicamentos: data.medicamentos || [],
      });
  ...
  function aplicar() {
    if (!extraido) return;
    onDiagnostico(extraido.diagnostico);
    const meds = extraido.medicamentos.map((m) => ({
      nome: m.nome,
      posologia: m.posologia || "",
      escola: false,
    }));
    onMedicamentos([...(peiData.lista_medicamentos || []), ...meds]);
    setExtraido(null);
    setFile(null);
  }
  ...
  return (
    <div className="p-4 rounded-xl border border-cyan-200 bg-cyan-50/50 space-y-3">
      <h4 className="font-medium text-slate-800">Laudo médico/escolar (PDF)</h4>
      <p className="text-sm text-slate-600">
        Anexe um laudo em PDF. A IA extrai diagnóstico e medicamentos para preencher o PEI.
      </p>
      <div className="flex flex-wrap gap-2 items-center">
        <input
          ...
          className="text-sm text-slate-600 file:mr-2 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-cyan-100 file:text-cyan-800"
        />
        <button
          type="button"
          onClick={extrair}
