...
export default async function MonitoramentoPage({ searchParams }: Props) {
  ...
  if (workspaceId && studentId && !student) {
    ...
    if (studentFromList) {
      console.warn("⚠️ Monitoramento: Estudante não encontrado por getStudent, mas encontrado na lista. Usando dados da lista.", {
        workspaceId,
        studentId,
      ...
      if (fullData && fullData.workspace_id === workspaceId) {
        ...
        console.warn("⚠️ Monitoramento: Usando dados básicos da lista", {
          studentId: student.id,
      ...
      console.error("❌ Monitoramento: Estudante não encontrado após getStudent e não está na lista", {
        workspaceId,
        studentId,
