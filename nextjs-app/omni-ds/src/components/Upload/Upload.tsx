    ...
    return (
        <div
            className={cn("relative flex flex-col items-center justify-center gap-2 p-8 border-2 border-dashed rounded-2xl cursor-pointer transition-all text-center",
                isDragging ? "border-sky-500 bg-sky-50 dark:bg-sky-950/20" : "border-(--omni-border-default) hover:border-sky-400 hover:bg-(--omni-bg-tertiary)", className
            )}
        ...
        >
            ...
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-(--omni-text-muted)"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>
            <p className="text-sm font-semibold text-(--omni-text-secondary)">{label}</p>
            {description && <p className="text-xs text-(--omni-text-muted)">{description}</p>}
        </div>
    );
}
export { Upload };
