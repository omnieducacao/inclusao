...
function StatCard({ title, value, icon, trend, color = "#0ea5e9", className }: StatCardProps) {
    return (
        <div className={cn("p-5 rounded-2xl border border-(--omni-border-default) bg-(--omni-bg-secondary) shadow-(--omni-shadow-md) hover:shadow-(--omni-shadow-elevated) hover:-translate-y-1 transition-all duration-200", className)}>
            <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-(--omni-text-muted)">{title}</span>
                {icon && <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: color }}>{icon}</div>}
            </div>
            <p className="text-3xl font-extrabold tracking-tight text-(--omni-text-primary)">{value}</p>
            {trend && (
                <div className="flex items-center gap-1 mt-2">
                    ...
                    {trend.label && <span className="text-xs text-(--omni-text-muted)">{trend.label}</span>}
                </div>
            )}
        </div>
    );
