...
function Progress({ value, max = 100, variant = "linear", size = "md", color = "#0ea5e9", showValue = true, label, className }: ProgressProps) {
    ...
    if (variant === "circular") {
        ...
        return (
            <div className={cn("inline-flex flex-col items-center gap-1", className)}>
                {label && <span className="text-xs font-semibold text-(--omni-text-muted)">{label}</span>}
    ...
    return (
        <div className={cn("w-full", className)}>
            ...
            <div className={cn("w-full rounded-full bg-(--omni-bg-tertiary) overflow-hidden", linearSizes[size])} role="progressbar" aria-valuenow={Math.round(pct)} aria-valuemin={0} aria-valuemax={100} aria-label={label || `Progresso: ${Math.round(pct)}%`}>
