                            ...
                            <div className="flex items-center gap-2 w-full">
                                {direction === "vertical" && (
                                    <div className="flex flex-col items-center">
                                        <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all shrink-0",
                                            status === "finished" ? "bg-sky-600 border-sky-600 text-white" :
                                            status === "active" ? "border-sky-600 text-sky-600 bg-transparent" :
                                            "border-(--omni-border-default) text-(--omni-text-muted) bg-transparent"
                                        )}>
                                        ...
                                        {!isLast && <div className={cn("w-0.5 flex-1 min-h-[24px] mt-1", status === "finished" ? "bg-sky-600" : "bg-(--omni-border-default)")} />}
                                    </div>
                                )}
                                {direction === "horizontal" && (
                                    <div className="flex items-center w-full">
                                        ...
                                        {!isLast && <div className={cn("flex-1 h-0.5 mx-2", i < current ? "bg-sky-600" : "bg-(--omni-border-default)")} />}
                                    </div>
                                )}
                            </div>
                            <div className={direction === "horizontal" ? "mt-2 text-center" : ""}>
                                <p className={cn("text-sm font-semibold", status === "active" ? "text-(--omni-text-primary)" : status === "finished" ? "text-(--omni-text-secondary)" : "text-(--omni-text-muted)")}>{item.title}</p>
                                {item.description && <p className="text-xs text-(--omni-text-muted) mt-0.5">{item.description}</p>}
                            </div>
                        </div>
                    </div>
                );
